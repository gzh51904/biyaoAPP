...
// Router.get('/', (req, res) => {
    // 爬取数据
    request('http://www.biyao.com/classify/category.html?categoryId=119', (err, response, body) => {
        // console.log(body);
        const $ = cheerio.load(body);
        let goodslist = [];
        $('.supplier-recommen li').each((idx,ele)=>{
            let $ele = $(ele);
            
            let name = $ele.find('.title').text();
            let supplier = $ele.find('.supplier').text();
            let price = $ele.find('.price').attr('price');
            // let pselling = $ele.find('.mack').text();
            let evaluate = $ele.find('a .evaluate').attr('evaluate');
            let imgurl = $ele.find('i img').attr('src');
            goodslist.push({
                       name,
                       supplier,
                        price,
        ...
        //     .pipe(fs.createWriteStream(`uploads/${filename}`))        $('.shopComBox li').each((idx, ele) => {
